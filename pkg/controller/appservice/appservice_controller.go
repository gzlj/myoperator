...
import (
	"fmt"
	appv1 "github.com/gzlj/myoperator/pkg/apis/app/v1"
	"github.com/gzlj/myoperator/pkg/resources"
...
func (r *ReconcileAppService) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	// old spec is from Annotations
	oldInstanceSpec := &appv1.AppServiceSpec{}
	...
	if !reflect.DeepEqual(instance.Spec, oldInstanceSpec) {
		// cr instance is already changed
		// create or update sub resource of cr instance
		...
		if err := r.client.Get(context.TODO(), request.NamespacedName, oldDeploy); err != nil {
			if errors.IsNotFound(err) {
				...
				deployNeedToUpdate = false
				deployDone = true
			} else {
				return reconcile.Result{}, err
			}
		}
		if deployNeedToUpdate {
		...
		oldServce := &corev1.Service{}
		servceNeedToUpdate := true

		if err := r.client.Get(context.TODO(), request.NamespacedName, oldServce); err != nil {
			if errors.IsNotFound(err) {
				//create service
				if err = r.client.Create(context.TODO(), newService); err != nil {
					fmt.Println("create service failed")
					reqLogger.Info("create service failed")
					reqLogger.Info(err.Error())
					fmt.Println(err)
					return reconcile.Result{}, err
				}
				servceNeedToUpdate = false
				serviceDone = true
			} else {
				return reconcile.Result{}, err
			}
		...
		if servceNeedToUpdate {
			clusterIp := oldServce.Spec.ClusterIP
			oldServce.Spec = newService.Spec
			oldServce.Spec.ClusterIP = clusterIp
			//update old service
			if err := r.client.Update(context.TODO(), oldServce); err != nil {
				reqLogger.Info("update service failed")
				return reconcile.Result{}, err
			}
			serviceDone = true
		}

		if deployDone && serviceDone {
			//update cr annotations
			data, _ := json.Marshal(instance.Spec)
			if instance.Annotations != nil {
				instance.Annotations["spec"] = string(data)
			} else {
